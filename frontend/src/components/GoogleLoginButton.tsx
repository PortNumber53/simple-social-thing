...
export const GoogleLoginButton: React.FC<GoogleLoginButtonProps> = ({
  ...
  const handleGoogleLogin = () => {
    ...
    const rawBackendOrigin = import.meta.env.VITE_BACKEND_ORIGIN;
    if (!rawBackendOrigin) {
      const msg = 'VITE_BACKEND_ORIGIN is not set. The frontend bundle was built without a backend origin, ' +
        'so Google OAuth cannot redirect to the API. Set VITE_BACKEND_ORIGIN (e.g. https://api-simple16.dev.portnumber53.com) ' +
    ...
    const backendUrl = rawBackendOrigin.replace(/\/+$/, '');
    ...
    return (
      <div className={`flex items-center gap-4 ${className}`}>
        <div className="flex items-center gap-3 px-4 py-2 bg-slate-100 dark:bg-slate-800 rounded-lg">
          <img
            src={sanitizeImageUrl(user.imageUrl, 'https://via.placeholder.com/150')}
            alt={user.name}
