...
export const ContentPublished: React.FC = () => {
  ...
  useEffect(() => {
    void loadAll();
    try {
      const stored = window.localStorage.getItem('publishedViewMode');
            ...
            <div className="sm:ml-auto">
              <div className="inline-flex rounded-md border border-slate-200 dark:border-slate-700 overflow-hidden">
                <button
                  ...
                  onClick={() => {
                    setViewMode('list');
                    try { window.localStorage.setItem('publishedViewMode', 'list'); } catch { /* ignore */ }
                  }}
                  className={[
                    'px-3 py-2 text-sm',
                    viewMode === 'list'
                ...
                <button
                  ...
                  onClick={() => {
                    setViewMode('gallery');
                    try { window.localStorage.setItem('publishedViewMode', 'gallery'); } catch { /* ignore */ }
                  }}
                  className={[
